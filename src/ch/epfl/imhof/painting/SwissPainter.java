    ...
    static {
        ...
        Painter roadPainter = RoadPainterGenerator.painterForRoads(
            new RoadSpec(tagged("highway", "motorway", "trunk"), 2, orange,
                    0.5f, black),
            new RoadSpec(tagged("highway", "primary"), 1.7f, lightRed, 0.35f,
                    black),
        ...
        Painter fgPainter = roadPainter.above(
            line(0.5f, darkGray, LineCap.Round, LineJoin.Miter, 1f, 2f).when(
                ...
                .above(
                    polygon(lightBlue).when(tagged("leisure", "swimming_pool")))
                .above(
                    line(0.7f, darkRed).when(
        ...
        Painter bgPainter = outline(1, darkBlue).above(polygon(lightBlue))
                .when(
                    tagged("natural", "water").or(
                ...
                .above(
                    polygon(lightGreen).when(
                        tagged("landuse", "grass", "recreation_ground",
                            "meadow", "cemetery").or(tagged("leisure", "park"))))
                .above(
